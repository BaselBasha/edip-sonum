                ...
                {items.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            />
                            <h2 className="text-lg font-bold mb-2 text-center">{t(item.name)}</h2>
                            ...
                            <p className="text-gray-600 text-center mb-4">{t(item.description || 'No description available')}</p>
                            <div className="relative flex items-center justify-between w-full mb-4">
                                <motion.div
                                    className="relative"
                                    ref={useMenuAnimation(isOpen)}
                                >
                                    <motion.button
                                        whileTap={{ scale: 0.97 }}
                                        onClick={() => {
                                            // Close any previously open menu and open the new one
                                    ...
                                    <motion.ul
                                        ...
                                        style={{ minWidth: '150px' }} // Adjust min-width as needed
                                    >
                                        {item.options.map((option) => (
                                            <motion.li
                                                ...
                                                style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} // Ensure text fits on one line
                                            >
                                                <span>{t(option.size)}</span>
                                                <span>{option.price} {t('tl')}</span>
                                            </motion.li>
                                        ))}
                                    </motion.ul>
                                </motion.div>
                                <button
                                    ...
                                    className="px-4 py-1.5 bg-green-500 text-white font-semibold rounded-md hover:bg-green-600 text-base ml-3"
                                >
                                    {t('Add to Cart')}
                                </button>
                            </div>
